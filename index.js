...
document.getElementById('games-checkbox').addEventListener('change', ($event) =>{
  ...
  }else{ hobbiesResult.children[1].classList.add("text-secondary")
  }
})

document.getElementById('music-checkbox').addEventListener('change', ($event) =>{
 if($event.target.checked) { hobbiesResult.children[2].classList.remove("text-secondary")
  }else{   hobbiesResult.children[2].classList.add("text-secondary");
  }
})
